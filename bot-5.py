...
def normalize_autopost_ids(save=True):
    """Keep saved Auto Post IDs continuous: 1, 2, 3...

    This also repairs older data that may still contain IDs such as 8, 9.
    """
    ...
    # Keep a stable order while repairing IDs.
    def sort_key(post):
        try:
    ...
    if int(AUTOPOST_DATA.get("next_id", 1)) != expected_next:
...
def add_autopost(time_hhmm, post_type, text="", photo_file_id=None):
    # ID always starts from 1 after the list is empty, and remains continuous.
    posts = AUTOPOST_DATA.setdefault("posts", [])
    normalize_autopost_ids(save=False)
    if not posts:
        post_id = 1
    else:
        post_id = max(int(p.get("id", 0)) for p in posts) + 1
...
def delete_autopost(post_id):
    """Delete a post and renumber the remaining posts from 1, 2, 3...

    This keeps the visible Auto Post IDs continuous. If all posts are deleted,
    the next newly added post starts again at ID 1.
    """
    posts = AUTOPOST_DATA.get("posts", [])
    before = len(posts)
    remaining = []
    ...
    if not deleted or len(remaining) == before:
        return False

    # Always keep IDs continuous: 1, 2, 3, ...
    for new_id, post in enumerate(
        sorted(remaining, key=lambda x: int(x.get("id", 0))),
        start=1,
    ):
        post["id"] = new_id

    AUTOPOST_DATA["posts"] = sorted(remaining, key=lambda x: int(x.get("id", 0)))
...
def format_autopost_list(post_id=None):
    ...
    # ID order-এ সব post দেখাবে, যাতে /list দিলে 1, 2, 3... সহজে দেখা যায়।
    lines = [
    ...
    lines.append("💡 নির্দিষ্ট পোস্ট দেখতে: /list ID")
    return "\n".join(lines)
...
async def autopost_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /list ID -> one post with Edit/Delete/Refresh buttons.
    if post_id is not None:
        post = find_autopost(post_id)
        if not post:
    ...
    posts = sorted(AUTOPOST_DATA.get("posts", []), key=lambda x: int(x.get("id", 0)))
    if not posts:
    ...
    await update.message.reply_text(
        ...
        "প্রতিটি পোস্টের নিচে Edit / Delete option আছে।",
        disable_web_page_preview=True,
    )
    for post in posts:
        ...
        await update.message.reply_text(
            autopost_detail_text(post),
            reply_markup=autopost_detail_keyboard(pid),
            disable_web_page_preview=True,
        )
...
async def autopost_command(
    ...
    if action == "list":
        ...
        # /autopost list ID -> one post with Edit/Delete/Refresh buttons.
        if post_id is not None:
            post = find_autopost(post_id)
            if not post:
                await update.message.reply_text(f"❌ ID {post_id} পাওয়া যায়নি।")
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(
            ...
            pattern=r"^autopost_(edit|delete|refresh|delete_yes|delete_no):\d+$",
        )
    )

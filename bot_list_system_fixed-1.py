...
async def autopost_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for post in posts:
        pid=int(post.get("id",0))
        ptype="📷 Photo" if post.get("type")=="photo" else "📝 Text"
        caption=(post.get("text") or "(ক্যাপশন নেই)").replace("\n"," ").strip()
        if len(caption)>80: caption=caption[:77]+"..."
        lines.append(f"🆔 ID {pid} • {ptype} • {caption}")
        row.append(InlineKeyboardButton(f"🆔 ID {pid}", callback_data=f"autopost_open:{pid}"))
        if len(row)==2:
            keyboard.append(row); row=[]
    if row: keyboard.append(row)
    keyboard.append([InlineKeyboardButton("🔄 Refresh List", callback_data="autopost_list")])
...
async def autopost_delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if delete_autopost(pid):
        await update.message.reply_text(
            f"✅ Auto Post ID {pid} delete হয়েছে।\n"
            "🔄 বাকি সব ID আবার 1, 2, 3... ধারাবাহিক হয়েছে।"
        )
    else:

...
def format_autopost_list():
    ...
    if not posts:
        return (
            "📋 RJ Team Bangladesh Auto Post List\n\n"
            f"⚙️ Status: {status}\n"
            "📌 Total Saved Post: 0\n\n"
            "❌ কোনো Saved Post নেই।"
        )
    ...
    lines = [
        ...
        "",
        f"⚙️ Status: {status}",
        f"📌 Total Saved Post: {len(posts)}",
        "",
    ]

    for post in sorted(posts, key=lambda x: int(x.get("id", 0))):
        pid = post.get("id", "-")
        ptype = "📸 Photo + Caption" if post.get("type") == "photo" else "📝 Text Post"
        caption = (post.get("text") or "").replace("\n", " ").strip()
        if len(caption) > 70:
            caption = caption[:70] + "..."

        lines.append(f"🆔 ID: {pid}")
        lines.append(f"⏰ Time: {display_time(post.get('time', ''))}")
        lines.append(f"📌 Type: {ptype}")
        if caption:
            lines.append(f"📝 {caption}")
        lines.append("")

    lines.append("💡 বিস্তারিত দেখতে: /list ID")
    lines.append("✏️ Edit/Delete করতে: /list ID")
    return "\n".join(lines)


def get_autopost(post_id):
...
def autopost_detail_text(post):
    ...
    return (
        ...
        f"📌 Type: {ptype}\n\n"
        "📝 Caption:\n"
        f"{caption if caption else '❌ Caption নেই'}"
    )


def autopost_action_keyboard(post_id):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✏️ Edit", callback_data=f"ap_edit_{post_id}"),
            InlineKeyboardButton("🗑️ Delete", callback_data=f"ap_del_{post_id}"),
        ],
        [
            InlineKeyboardButton("🔄 Refresh", callback_data=f"ap_view_{post_id}"),
        ],
    ])
...
async def autopost_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /list        -> সব saved post
    # /list 1      -> ID 1-এর বিস্তারিত + Edit/Delete buttons
    post_id = None
    if context.args and context.args[0].isdigit():
        post_id = int(context.args[0])

    if post_id is not None:
        post = get_autopost(post_id)
        if not post:
        ...
        await update.message.reply_text(
            ...
            reply_markup=autopost_action_keyboard(post_id),
            disable_web_page_preview=True,
        )
        return
    ...
    # একবার /list দিলে সব saved post দেখাবে; প্রতিটির সাথে Edit/Delete থাকবে।
    for post in posts:
        pid = int(post.get("id"))
        await update.message.reply_text(
            autopost_detail_text(post),
            reply_markup=autopost_action_keyboard(pid),
            disable_web_page_preview=True,
        )
...
async def autopost_command(
    ...
    if not args:
        await update.message.reply_text(
            ...
            "/autopost list - সব saved post + Edit/Delete button\n"
            "/autopost list 1 - ID 1-এর বিস্তারিত + Edit/Delete\n"
            ...
            "\nShortcuts:\n"
            "/list /list 1 /delete 1 /on /off /test\n"
            "\nনতুন format-ও চলবে:\n"
            "10:30 PM|আপনার Caption"
        )
        return
...
async def autopost_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = (user.username or "").strip().lstrip("@")
    allowed = (
        (ADMIN_USER_ID and str(user.id) == ADMIN_USER_ID)
        or (ADMIN_USERNAME and username.lower() == ADMIN_USERNAME.lower())
    )
    ...
    if data.startswith("ap_view_"):
        try:
            post_id = int(data.split("_")[-1])
        except ValueError:
            await query.message.reply_text("❌ ভুল Post ID।")
            return

        post = get_autopost(post_id)
        if not post:
            await query.message.reply_text(f"❌ ID {post_id} পাওয়া যায়নি।")
            return
    ...
    if data.startswith("ap_edit_"):
        ...
        context.user_data["editing_autopost_id"] = post_id

        await query.message.reply_text(
            f"✏️ ID {post_id} Edit Mode চালু হয়েছে।\n\n"
            "নতুন Time + Caption এই format-এ পাঠান:\n\n"
    ...
    if data.startswith("ap_del_"):
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "✅ হ্যাঁ, Delete",
                    callback_data=f"ap_confirm_del_{post_id}",
                ),
                InlineKeyboardButton(
                    "❌ Cancel",
                    callback_data=f"ap_view_{post_id}",
                ),
            ]
        ])

        await query.message.reply_text(
            f"⚠️ ID {post_id} delete করতে চান?\n"
            "Delete করলে এই scheduled post আর automatic post হবে না।",
            reply_markup=keyboard,
        )
        return

    if data.startswith("ap_confirm_del_"):
        try:
            post_id = int(data.split("_")[-1])
        except ValueError:
            await query.message.reply_text("❌ ভুল Post ID।")
        ...
        if delete_autopost(post_id):
            context.user_data.pop("editing_autopost_id", None)
            await query.message.reply_text(
                f"🗑️ ID {post_id} সফলভাবে Delete হয়েছে।"
            )
        else:
            await query.message.reply_text(f"❌ ID {post_id} পাওয়া যায়নি।")
        return
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.pop("editing_autopost_id", None)
    if update.message:
        await update.message.reply_text("❌ Edit mode বন্ধ করা হয়েছে।")
...
async def handle_message(
    ...
    # -----------------------------------------------------
    # ADMIN AUTO POST EDIT MODE
    # -----------------------------------------------------
    editing_id = context.user_data.get("editing_autopost_id")
    if editing_id and is_admin(update):
        ...
        hhmm = parse_ampm_time(time_part.strip())
        caption = caption.strip()

        if not hhmm:
            await update.message.reply_text(
                "❌ সময় ঠিক নয়। উদাহরণ: 10:30 PM"
            )
            return
        ...
        post = get_autopost(editing_id)
        if not post:
            context.user_data.pop("editing_autopost_id", None)
            await update.message.reply_text(
                f"❌ ID {editing_id} পাওয়া যায়নি।"
            )
            return

        max_len = 1024 if post.get("type") == "photo" else 4096
        if len(caption) > max_len:
            await update.message.reply_text(
                f"❌ এই Post-এর Caption সর্বোচ্চ {max_len} character হতে পারে।"
            )
            return
        ...
        await update.message.reply_text(
            f"✅ ID {editing_id} সফলভাবে Update হয়েছে।\n\n"
            + autopost_detail_text(post),
            reply_markup=autopost_action_keyboard(editing_id),
        )
        return

    # -----------------------------------------------------
    # ADMIN DIRECT AUTO POST FORMAT
    # Example: 10:30 PM|শুভ রাত্রি 🌙
    # -----------------------------------------------------
    if is_admin(update) and "|" in text:
        time_part, caption = text.split("|", 1)
        hhmm = parse_ampm_time(time_part.strip())
        caption = caption.strip()
        ...
        if hhmm and caption:
            if len(caption) > 4096:
                await update.message.reply_text(
                    "❌ Text post সর্বোচ্চ 4096 character হতে পারে।"
                )
                return

            post_id = add_autopost(hhmm, "text", text=caption)
            await update.message.reply_text(
                f"✅ Auto Post Save হয়েছে।\n"
                f"🆔 ID: {post_id}\n"
                f"⏰ Time: {display_time(hhmm)}\n\n"
                f"📋 বিস্তারিত: /list {post_id}"
            )
            return

...
async def autopost_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = format_autopost_list(post_id)
    chunks = split_long_text(text)

    for index, chunk in enumerate(chunks):
        # ID-specific list-এ inline Edit/Delete button দেখাবে।
        if post_id is not None and index == len(chunks) - 1 and find_autopost(post_id):
            await update.message.reply_text(
                chunk,

...
async def autopost_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            ...
            await update.message.reply_text("❌ ID ঠিক নয়। উদাহরণ: /list 4")
            return
    ...
    lines = [
        ...
        f"📦 Total: {len(posts)}",
        "",
        "👇 ID টাচ করলে পুরো saved post/caption খুলবে:",
    ]
...
async def autopost_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (query.data or "").startswith("autopost_open:"):
        ...
        # Photo posts: show the actual saved Telegram photo + caption.
        if post.get("type") == "photo" and post.get("photo_file_id"):
            try:
                await query.message.reply_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [
                            InlineKeyboardButton(
                                "✏️ Edit",
                                callback_data=f"autopost_edit:{post_id}",
                            ),
                            InlineKeyboardButton(
                                "🗑️ Delete",
                                callback_data=f"autopost_delete:{post_id}",
                            ),
                        ],
                        [
                            InlineKeyboardButton(
                                "🔄 Refresh",
                                callback_data=f"autopost_refresh:{post_id}",
                            ),
                        ],
                        [
                            InlineKeyboardButton(
